...
class RestInterface:
    ...
    __slots__ = ("host", "https", "ports", "_session", "lastResults", "verifySSL")
    ...
    def _post(self, apiUrl, jsonData):
        ...
        response = self.session.post(
            url, data=payload, verify=(self.https and self.verifySSL)
        )
    ...
    def _raiseUnhandledPostError(self, apiUrl, jsonData):
        ...
        if self.lastResults["errors"]:
            if jsonData and "password" in jsonData:
                jsonData["password"] = "********"
            raise RuntimeError(
                'Canary API call to "%s" had errors: %r.\nData passed: %r'
