...
class CanarySender(SessionTokenManagement, UserTokenManagement):
    ...
    def storeData(self, tvqs={}, properties={}, annotations={}, maxPageSize=25000):
        ...
        if (
            sum(
                sum(len(values) for values in tagDict.values())
        ...
        self._lastStoredTags = (
            set(tvqs.keys()) | set(properties.keys()) | set(annotations.keys())
        )
    ...
    def noData(self, tags=[]):
        ...
        if not tags:
            assert (
                self._lastStoredTags
            ), "Can't set 'No Data' without context. No tags given and _lastStoredTags is empty."
    ...
    def getDatasets(self, historian):
        ...
        """
        jsonData = {"userToken": self.userToken, "historian": historian}
