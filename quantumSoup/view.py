...
class CanaryView(LiveDataTokenManagement, UserTokenManagement):
    ...
    def browseNodes(self, path=""):
        ...
        """
        jsonData = {"userToken": self.userToken, "path": path}
    ...
    def browseStatus(self, views):
        ...
        """
        jsonData = {"userToken": self.userToken, "views": self._coerceToList(views)}
        ...
        if isinstance(views, str):
            ...
            return (
                (viewName, statuses.get(viewName, {}).get("sequence", None))
    ...
    def getTagProperties(self, tags):
        ...
        """
        jsonData = {"userToken": self.userToken, "tags": self._coerceToList(tags)}
    ...
    def getTagData(self, tags, **constraints):
        ...
        if isinstance(tags, str):
            ...
            for valueChunk in self._getTagData(tagPath, **constraints):
                ...
                if valueChunk.get(tagPath, []) and not isinstance(
                    valueChunk[tagPath][0], list
                ):
    ...
    def getLiveData(self, tags=None, **configuration):
        ...
        if not tags:
            if not len(self._liveDataTokens) == 1:
                raise ValueError(
                    "getLiveData() called without a tag list _and_ did not have only one live data token."
                )
    ...
    def _post(self, apiUrl, jsonData):
        ...
        if self.lastResults["statusCode"] == "BadLicense":
            raise RuntimeError(
                "The target Canary instance is not licensed for third party View usage."
            )
