...
class PasswordStorage:
    """Simple in-memory password storage"""
    ...
    def delete_password(cls, service, username):
        cls.__storage.pop((service, username), None)


class UserTokenManagement(RestInterface):

    __slots__ = ("_userToken", "_username", "_apiToken", "__clear_on_exit")

    def __init__(self, username="", password="", apiToken=None, **configuration):
        ...
        if apiToken:
            self._username = "apiToken"
            PasswordStorage.set_password("birdsong", self.__self_id, apiToken)
        elif username and password:
            PasswordStorage.set_password("birdsong", self.__self_id, password)
        else:
            raise ValueError(
                "Either apiToken or both username and password must be provided"
            )
    ...
    @property
    def __password(self):
        return PasswordStorage.get_password("birdsong", self.__self_id)
    ...
    def _getUserToken(self):
        if self._apiToken:
            jsonData = {"apiToken": self._apiToken, "application": "getData"}
        else:
            jsonData = {
    ...
    def __exit__(self, *args):
        ...
        if self._username:
            self._username = ""
            PasswordStorage.delete_password("birdsong", self.__self_id)
    ...
    def _post(self, apiUrl, jsonData):
        ...
        if self.lastResults["statusCode"] == "BadUserToken":
            assert "userToken" in jsonData, (
                "API '%s' called with bad user token without including one." % apiUrl
            )
            self._userToken = None
            self._getUserToken()
...
class LiveDataTokenManagement(UserTokenManagement, RestInterface):
    ...
    def __init__(
        ...
        **configuration
        ...
        if liveTags:
            self._getLiveDataToken(
                liveTags, mode=liveMode, includeQuality=liveIncludeQuality
            )
    ...
    def _getLiveDataToken(self, tags, **configuration):
        if isinstance(tags, frozenset) and tags in self._liveDataTokens:
            return
        jsonData = {"userToken": self._userToken, "tags": self._coerceToList(tags)}
        jsonData.update(configuration)
        tagSet = frozenset(jsonData["tags"])
        if not tagSet in self._liveDataTokens:
            self._liveDataTokens[tagSet] = self._singlePost(
                "getLiveDataToken", jsonData, "liveDataToken"
            )
    ...
    def _rotateLiveDataToken(self, liveDataToken):
        ...
        for tagSet, activeLiveDataToken in self._liveDataTokens.items():
            ...
            raise KeyError(
                "The liveDataToken could not be rotated because it was not cached."
            )
    ...
    def _post(self, apiUrl, jsonData):
        ...
        if self.lastResults["statusCode"] == "BadLiveDataToken":
            assert "liveDataToken" in jsonData, (
                "API '%s' called with bad user token without including one." % apiUrl
            )

            jsonData["liveDataToken"] = self._rotateLiveDataToken(
                jsonData["liveDataToken"]
            )
            # Try again
            self._post(apiUrl, jsonData)
...
class SessionTokenManagement(UserTokenManagement, RestInterface):
    ...
    def _getSessionToken(self):
        ...
        self._sessionToken = self._singlePost(
            "getSessionToken", jsonData, "sessionToken"
        )
    ...
    def getErrors(self):
        assert (
            self._sessionToken
        ), "Session token missing - can't get errors of missing session."
        jsonData = {"userToken": self._userToken, "sessionToken": self._sessionToken}
        self._singlePost("getErrors", jsonData)

    def keepAlive(self):
        jsonData = {"userToken": self.userToken, "sessionToken": self.sessionToken}
    ...
    def _post(self, apiUrl, jsonData):
        ...
        # Check if it failed. If so, reload.
        if self.lastResults["statusCode"] == "BadSessionToken" or (
            self.lastResults["statusCode"] == "Error"
            and "Session token is invalid or has expired" in self.lastResults["errors"]
        ):
            assert "sessionToken" in jsonData, (
                "API '%s' called with bad session token without including one." % apiUrl
            )
            self._sessionToken = None
            self._getSessionToken()
